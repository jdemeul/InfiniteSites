...
annotate_spectrum <- function(sampleid, trinucs, simsubset = "1plus1") {
  ...
  simfile <- paste0("/srv/shared/vanloo/home/jdemeul/projects/2016-17_ICGC/infinite_sites/results/isabreakdown_", simsubset, "_only_writefrac/", sampleid, "/", paste0(sampleid, "_infsites_permut_effgenfrac1.txt"))
  ...
  rmat <- MCMCpack::rdirichlet(n = 10000, alpha = plotdf$parobs+plotdf$allobs*96/sum(plotdf$allobs))
  ...
  plotdf$allobs_norm <- exp(log(plotdf$allobs) - log(trinucs[plotdf$trinuc]))
  plotdf$allobs_norm <- plotdf$allobs_norm/sum(plotdf$allobs_norm)
  # plotdf$allobs_norm <- plotdf$allobs/sum(plotdf$allobs)
  
  plotdf$sim_norm <- exp(log(plotdf$counts_tot) - log(trinucs[plotdf$trinuc]))
  ...
  parcossim <- quantile(apply(X = rmat, MARGIN = 1, FUN = function(x, y) sum(x*y)/(sqrt(sum(x^2))*sqrt(sum(y^2))), y = plotdf$counts_tot), probs = c(.025, .5, .975))
  ...
  parcosall <- quantile(apply(X = rmat, MARGIN = 1, FUN = function(x, y) sum(x*y)/(sqrt(sum(x^2))*sqrt(sum(y^2))), y = plotdf$allobs), probs = c(.025, .5, .975))
  ...
  p1 <- ggplot(data = plotdf, mapping = aes(x = mut1)) + 
    geom_col(mapping = aes(y = allobs_norm, fill = type), show.legend = F, alpha = .8) +
  ...
  p1 <- p1 + geom_segment(data = data.frame(start = seq(0.65, 96, 16), end = seq(16.35, 97, 16), type = bases$types2),
                          mapping = aes(x = start, xend = end, y = 1.25*max(plotdf$pobs_hi), yend = 1.25*max(plotdf$pobs_hi), color = type), size = 3, show.legend = F, alpha = .8)
  ...
  ggsave(filename = paste0(outfile, ".pdf"), plot = p1, width = 16, height = 4)
  ...
  if (nrow(thirdhitsdf) == 0) return(NULL)
  ...
  plotdf2$allobs_norm <- exp(log(plotdf2$allobs) - log(trinucs[plotdf2$trinuc]))
  plotdf2$allobs_norm <- plotdf2$allobs_norm/sum(plotdf2$allobs_norm)
  # plotdf$allobs_norm <- plotdf$allobs/sum(plotdf$allobs)
  
  plotdf2$sim_norm <- exp(log(plotdf2$counts_tot) - log(trinucs[plotdf2$trinuc]))
  ...
  thirdcossim <- quantile(apply(X = rmat, MARGIN = 1, FUN = function(x, y) sum(x*y)/(sqrt(sum(x^2))*sqrt(sum(y^2))), y = plotdf2$counts_tot), probs = c(.025, .5, .975))
  
  # cosall <- sum(sum(as.numeric(plotdf2$thirdobs)*as.numeric(plotdf2$allobs)))/(sqrt(sum(plotdf2$thirdobs^2))*sqrt(sum(plotdf2$allobs^2)))
  thirdcosall <- quantile(apply(X = rmat, MARGIN = 1, FUN = function(x, y) sum(x*y)/(sqrt(sum(x^2))*sqrt(sum(y^2))), y = plotdf2$allobs), probs = c(.025, .5, .975))
  ...
  p1 <- p1 + geom_segment(data = data.frame(start = seq(0.65, 96, 16), end = seq(16.35, 97, 16), type = paste0(bases$types2, "+", c("G", "T", "A", "C", "G", "A"))),
                          mapping = aes(x = start, xend = end, y = 1.25*max(plotdf2$pobs_hi), yend = 1.25*max(plotdf2$pobs_hi), color = type), size = 3, show.legend = F, alpha = .8)
  p1 <- p1 + geom_text(data = data.frame(type = paste0(bases$types2, "+", c("G", "T", "A", "C", "G", "A")), pos = seq(8, 96, 16)),
                       ...
                       show.legend = F, family = "mono")
  p1 <- p1 + theme_bw() + scale_x_discrete(labels = plotdf2$trinuc) + scale_fill_manual(values = c("C>A+G" = "#15bcee","C>G+T" = "#000000","C>T+A" = "#e32926","T>A+C" = "#999999","T>C+G" = "#a1ce63","T>G+A" = "#ebc6c4")) +
...
sampleid <- "5c3def3a-b515-41f6-8157-681b963534e7"
