...
# sampleid <- "60413de1-6cd2-4f74-8180-3bdd394d6d16"
# sampleid <- "2df02f2b-9f1c-4249-b3b4-b03079cd97d9"
sampleid <- "0980e7fd-051d-45e9-9ca6-2baf073da4e8"

##### check Mutect2 recalled variants for multiallelics (otherwise PASS variants)
vcffilepath <- list.files(path = "/srv/shared/vanloo/home/jdemeul/projects/2016-17_ICGC/infinite_sites/data/pcawg_recall/", pattern = paste0(sampleid, "_tumor_mutect2_snvs_indels_oncefiltered.vcf.gz$"), full.names = T, recursive = T)
snvs <- readVcf(file = vcffilepath, genome = seqinfo(BSgenome.Hsapiens.1000genomes.hs37d5))
snvs_pass <- snvs[seqnames(snvs) %in% c(1:22, "X", "Y") & rowRanges(snvs)$FILTER == "PASS" & nchar(ref(snvs)) == 1]
snvs_compare <- snvs_pass[lengths(alt(snvs_pass)) == 1]
snvs_compare <- snvs_compare[unlist(nchar(alt(snvs_compare))) == 1]
...
c(onlym2 = sum(!overlapsAny(query = snvs_compare, subject = snvs_pcawg, type = "equal")),
  both = sum(overlapsAny(query = snvs_compare, subject = snvs_pcawg, type = "equal")),
  onlycons = sum(!overlapsAny(query = snvs_pcawg, subject = snvs_compare, type = "equal")))
...
# get read depths
# normal
geno(snvs_pass_biallelic[9])$F1R2[[2]] + geno(snvs_pass_biallelic[9])$F2R1[[2]]
# tumour
geno(snvs_pass_biallelic[9])$F1R2[[1]] + geno(snvs_pass_biallelic[9])$F2R1[[1]]
...
alhitssim <- read.delim(file = paste0("/srv/shared/vanloo/home/jdemeul/projects/2016-17_ICGC/infinite_sites/results/isabreakdown/", sampleid, "_infsites_backfwd_allelic.txt"), as.is = T)
